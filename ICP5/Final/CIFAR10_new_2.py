...
INIT_LR = 5e-3
model.compile(optimizer=tf.keras.optimizers.Adamax(lr=INIT_LR),loss='categorical_crossentropy',
              metrics=['accuracy'])
...
history=model.fit(x_train, y_train, batch_size=512, epochs=40, validation_data=(x_test, y_test))
#training accuracy without dropout
train_acc=model.evaluate(x_train,y_train,batch_size=256)
...
model2.compile(loss='categorical_crossentropy', metrics=['accuracy'],optimizer=tf.keras.optimizers.Adamax(lr=INIT_LR))

# train with image augmentation
history2=model2.fit_generator(datagen.flow(x_train, y_train, batch_size=512),
                    steps_per_epoch = len(x_train) / 512, epochs=40, validation_data=(x_test, y_test))

model2_train_acc=model2.evaluate(x_train,y_train,batch_size=256)
print("Training Accuracy with Data Augumentation={}".format(model2_train_acc))


model2_test_acc=model2.evaluate(x_test,y_test,batch_size=256)
...
model = Sequential()
ac = 'relu'
model.add(Conv2D(num_filters, (3, 3), activation=ac, input_shape=(img_rows, img_cols, channels)))
model.add(MaxPooling2D(2,2))   # reduces to 16x16x3xnum_filte

model.add(Conv2D(num_filters, (3, 3), activation=ac))
model.add(MaxPooling2D(2,2))   # reduces to 8x8x3x(2*num_filters)

model.add(Conv2D(num_filters, (3, 3), activation=ac))
model.add(Flatten())
model.add(Dense(64, activation=ac))
